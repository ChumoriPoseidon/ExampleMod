...
public class ExampleDecomposer extends ContainerBlock {

	public static final DirectionProperty FACING = HorizontalBlock.HORIZONTAL_FACING;
	public static final BooleanProperty LIT = BlockStateProperties.LIT;

	public ExampleDecomposer() {
		super(Properties.create(Material.ROCK).sound(SoundType.STONE).hardnessAndResistance(3.5F, 3.5F));
		this.setRegistryName("block_example_decomposer");
	...
	public TileEntity createNewTileEntity(IBlockReader worldIn) {
		return new ExampleDecomposerTileEntity();
	}
	...
	public boolean onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
		if(!worldIn.isRemote) {
			...
			if(tileEntity instanceof ExampleDecomposerTileEntity) {
	...
	public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
		if (state.getBlock() != newState.getBlock()) {
			TileEntity tileEntity = worldIn.getTileEntity(pos);
			if(tileEntity instanceof ExampleDecomposerTileEntity) {
				InventoryHelper.dropInventoryItems(worldIn, pos, (ExampleDecomposerTileEntity)tileEntity);
